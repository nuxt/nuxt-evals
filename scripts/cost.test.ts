/**
 * Tests for token extraction + pricing. Run with: pnpm run test:cost
 *
...
test('codex: single turn.completed, input includes cached', () => {
  ...
  // uncached input = 1000 - 800; cacheRead = 800; output as reported
...
test('every experiment has a MODEL_PRICING entry', () => {
  const experiments = readdirSync(join(import.meta.dirname, '..', 'experiments'))
    .filter((f) => f.endsWith('.ts'))
    .map((f) => f.replace(/\.ts$/, ''));
  const missing = experiments.filter((e) => !(e in MODEL_PRICING));
  assert.deepEqual(missing, [], `add these to MODEL_PRICING or /evals shows N/A: ${missing.join(', ')}`);
});
