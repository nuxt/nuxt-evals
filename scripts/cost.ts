/**
 * List-cost estimation for nuxt.com/evals.
 *
...
/**
 * List prices per 1M tokens, keyed by experiment slug.
 *
 * Values are a snapshot of the AI Gateway model catalog
 * (`https://ai-gateway.vercel.sh/v1/models`, base tier — the tiered
 * long-context rates above 200k–272k tokens are not applied). Cursor Composer
 * doesn't run through the gateway; its rates come from
 * cursor.com/docs/models-and-pricing (Standard tier).
 *
 * null = do not estimate a cost (rendered as N/A) — for a harness that writes
 * no token usage to its transcript, so there is nothing to price.
 *
...
export const MODEL_PRICING: Record<string, Pricing | null> = {
  ...
  // gemini-3-pro-preview is no longer listed on the gateway; last-known rate.
 ...
 * if the transcript carries no usage (e.g. a timed-out run saves none).
 *
